...
export class StoreproductsService {
    ...
    async updateStoreProduct(StoreId: number, ProductId: number, Quantity: number): Promise<any> {
        try {
            const result = await this.storeproductsRepository.save({ StoreId: StoreId, ProductId: ProductId, Quantity: Quantity });
            return result;
        } catch (error) {
    ...
    async decreaseStoreProductQuantity(UserId: number, ProductId: number, Quantity: number): Promise<any> {
        try {
            ...
            const result = await this.storeproductsRepository.save({ StoreId: acc.StoreId, ProductId: ProductId, Quantity: updateValue });
            return result;
        } catch (error) {
            customThrowError(RESPONSE_MESSAGES.ERROR, HttpStatus.BAD_REQUEST, error);
        }
    ...
    async addProductToStore(ProductId: number, UserId: number): Promise<any> {
        try {
            const acc = await this.accountsService.findOneById(UserId);
            const result = await this.storeproductsRepository.save({ StoreId: acc.StoreId, ProductId: ProductId, Quantity: 0 });
            return result;
        } catch (error) {
            customThrowError(RESPONSE_MESSAGES.ERROR, HttpStatus.BAD_REQUEST, error);
        }
