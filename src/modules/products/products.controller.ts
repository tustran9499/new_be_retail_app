...
export class ProductsController {
  ...
  async updateProduct(
    ...
    if (req.user.role == "StoresManager") {
      return this.ProductsService.updateProductByAdmin(
        req.user.userId,
        id,
        model
      );
    } else {
      return this.ProductsService.updateProductByStore(
        req.user.userId,
        id,
        model
      );
  ...
  @ApiOkResponse()
  getProductByBarcode(@Param("barcode") barcode: string): Promise<any> {
